...
while salir==0:
    ...
    print('2) Enviar mensaje a Daniel')
    print('3) Enviar mensaje a William')
    print('4) Enviar mensaje a Manuel')    
    print('5) Salir')
    ...
    if opcion==1:
        ...
        if usuario=='192.168.0.13': #Cifrados otros
            print('Texto descirado otros: {}'.format(desotro1(recibido)))
        ...
        if usuario == '192.168.0.7': #Cifrado Cesar
            print('Texto descifrado cesar: {}'.format(descesarr(recibido)))
            #Devolvemos el mensaje al cliente
            cli.send("mensaje recibido".encode("UTF-8"))
        ...
        if  (usuario=="192.168.0.15"): #cifrado Gill
            print("mensaje de daniel")
            filas=0
           ...
           #print("Texto cifrado:")
            ...
            print('Texto descifrado hill: '+mensajecodificado)
    ...
    if opcion==2:
        ...
        for i in range (filas):
            ...
            i=i+1
        mensajecodificado=''.join(mensaje)
        print('Texto cifrado hill: '+mensajecodificado)
    ...
    if opcion==3:
        ...
        print('Texto cigrado cesar: {}'.format(cesarr(mens)))
        #Con el método send, enviamos el mensaje
        obj.send(cesarr(mens).encode('UTF-8'))
        recivido = obj.recv(1024)
        print(recivido.decode('UTF-8'))
        #Cerramos la instancia del objeto servidor
        obj.close()
    ...
    if opcion==4:
        ...
        #Instanciamos una entrada de datos para que el cliente pueda enviar mensajes
        mens = input("Ingrese palabra a enviar: ")
        print('Palabra cifrada otro: {}'.format(otro1(mens)))
        #Con el método send, enviamos el mensaje
        obj.send(otro1(mens).encode('UTF-8'))
        recivido = obj.recv(1024)
        print(recivido.decode('UTF-8'))
        #Cerramos la instancia del objeto servidor
        obj.close()
